...
class _PatientDashboardPageState extends State<PatientDashboardPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return PatientLayout(
      ...
      child: Column(
        children: [
          if (pacienteActual.estado != 'Activo') ...[
            Container(
              ...
              child: const Text(
                'Tu perfil está inactivo. No puedes crear, editar ni cancelar acciones hasta que el administrador reactive tu cuenta.',
                style: TextStyle(
                  color: Colors.red,
  ...
  void confirmarSolicitudPaciente(String tipoSolicitud, Color color) {
    if (pacienteActual.estado != 'Activo') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Paciente inactivo. No puede realizar solicitudes.'),
  ...
  Widget requestButton(String text, IconData icon, Color color) {
    return InkWell(
      borderRadius: BorderRadius.circular(10),
      onTap: pacienteActual.estado == 'Activo'
          ? () => confirmarSolicitudPaciente(text, color)
          : () => ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  'Paciente inactivo. No puede realizar solicitudes.',
                ),
                backgroundColor: Colors.red,
              ),
            ),
...
class _PatientRequestsPageState extends State<PatientRequestsPage> {
  ...
  Widget build(BuildContext context) {
    final misSolicitudes = solicitudes
        .where(
          (s) =>
              s.paciente == pacienteActual.nombre &&
              ['Completada', 'Reasignada', 'Cancelada'].contains(s.estado),
        )
        .toList();
    return PatientLayout(
      title: 'Historial de Solicitudes',
      subtitle:
          'Solo se muestran solicitudes completadas, reasignadas o canceladas',
      child: Column(
        children: [
          if (misSolicitudes.isEmpty)
            Container(
              ...
              child: const Center(
                child: Text(
                  'Aún no tienes solicitudes completadas, reasignadas o canceladas en el historial.',
                ),
              ),
            ),
          ...misSolicitudes.map(
          ...
          children: [
            ...
            SizedBox(
              ...
              child: FilledButton.icon(
                onPressed: pacienteActual.estado == 'Activo'
                    ? guardarInformacion
                    : null,
  ...
  Widget patientInput(String label, TextEditingController controller) {
    return Padding(
      ...
      child: TextField(
        ...
        enabled: pacienteActual.estado == 'Activo',
...
class _EmergencyContactsPageState extends State<EmergencyContactsPage> {
  ...
  void eliminarContacto(BuildContext context, ContactoEmergencia contacto) {
    if (pacienteActual.estado != 'Activo') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Paciente inactivo. No puede eliminar contactos.'),
          backgroundColor: Colors.red,
        ),
      );
      return;
  ...
  void editarContacto(BuildContext context, ContactoEmergencia? contacto) {
    if (pacienteActual.estado != 'Activo') {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Paciente inactivo. No puede modificar contactos.'),
          backgroundColor: Colors.red,
        ),
      );
      return;
